...
- Identity: email + scrypt password hash and/or Google account id in SQLite (`.data/app.db` by default, or `AUTH_DATA_DIR/app.db`). HMAC-signed `rs_session` cookie. On first boot, if the DB is empty and `.data/users.json` / `.data/training.json` exist, they are imported once; SQLite is then the source of truth. `users.emailVerifiedAt` is nullable and is never backfilled — existing accounts stay unverified. Password signup and password login do not set it. A successful Google callback sets it only when Google reports `email_verified` (userinfo or id token). Consuming a magic link sets it. When an unverified account becomes verified, its password hash is cleared and its other sessions stop working; an already-verified account keeps its password.
