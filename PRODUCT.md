...
- **Today (`/today`):** the Session whose `date` equals today’s Guayaquil date. Empty copy: **No session today**. Skip / Feeling off persist Feedback immediately (Skip also sets session `outcome`) and never show a map. **Done** opens a bottom sheet and does not persist until **Save** or **Skip map**. Save writes Feedback (`kind: "done"`) plus a `RunLog`. Skip map writes Feedback only (no `RunLog`). Planned `Session.distanceKm` is unchanged. AdaptationEvents are loaded for display only (chip `title` + `summary`; Why? disabled; `reason` in data; never written here).
