...
- Today shows the session dated for that Guayaquil calendar day. Empty copy is **No session today**. Skip / Feeling off write Feedback immediately. **Done** opens a bottom sheet **Log this run** (map + editable distance / time / pace); **Save** writes Feedback and logged run stats. AdaptationEvents are read-only in the shell (chip = `title` + `summary`; Why? stays disabled; `reason` is stored). Written only by the 21:00 job, and only when Feedback exists that day.
...
- **Today (`/today`):** the Session whose `date` equals today’s Guayaquil date. Empty copy: **No session today**. Skip / Feeling off persist Feedback immediately (Skip also sets session `outcome`). **Done** does not persist until **Save** on the **Log this run** sheet. Save writes Feedback (`kind: "done"`) plus `loggedRun` on that Feedback and the Session (`distanceKm`, `timeSec`, `paceSecPerKm` computed server-side as `timeSec / distanceKm`, `route`). Planned `Session.distanceKm` is unchanged. Skip and Feeling off never open the sheet. AdaptationEvents are loaded for display only (chip `title` + `summary`; Why? disabled; `reason` in data; never written here).
- **Log this run (Done only):** mobile-first bottom sheet (~390). Map shows a GPS polyline when `route.kind === "polyline"`; otherwise a pin and **No route yet**. Distance prefills from the Session when present; time and pace start empty. Pace is editable in the UI and recomputed server-side from distance + time (client pace is not trusted). Time `> 0` is required (`hh:mm:ss`, `mm:ss` accepted). Distance 0.1–100 km. Pace outside ~150–720 s/km (~2:30–12:00 /km) is a soft warning and does not block Save. `route` is `{ kind: "none" }` for MVP (no GPS hardware); polyline points can be stored later. No Strava attach. After Save, Today shows the map and logged stats.
- **Plan (`/plan`):** week strip M–S for the Guayaquil week (Monday–Sunday) and up to three remaining sessions this week.
- **Progress (`/progress`):** labels **Consistency**, **Easy pace**, **Weekly distance**. Consistency and weekly distance come from this week’s planned sessions. Easy pace uses this week’s logged easy-run paces when present; otherwise it stays provisional.
