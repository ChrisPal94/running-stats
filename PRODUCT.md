...
Users evaluate the product on a marketing landing page, then start or return to a plan with email and password, an email magic link, or Google. Signed-in users complete a five-step onboarding that creates Plan v1, then land in the Hoy / Plan / Progreso app shell.
...
- Auth is email + password, email magic link, and Google (no Strava). The stored identity is the user’s.
- After signup (email, first magic link, or first Google), continue to `/onboarding`. After login (email, returning magic link, or returning Google), continue to `/today` when a plan exists, otherwise `/onboarding`. Shell routes send signed-in users without a plan back to onboarding.
...
- Routes: `/signup`, `/login`, `/logout` (POST), `/auth/google`, `/auth/google/callback`, `/auth/magic`, `/onboarding`, `/today`, `/plan`, `/progress`, `/settings`.
...
- Identity: email + scrypt password hash and/or Google account id in SQLite (`.data/app.db` by default, or `AUTH_DATA_DIR/app.db`). HMAC-signed `rs_session` cookie. On first boot, if the DB is empty and `.data/users.json` / `.data/training.json` exist, they are imported once; SQLite is then the source of truth. `users.emailVerifiedAt` is nullable and is never backfilled — existing accounts stay unverified. Password signup and password login do not set it. A successful Google callback sets it only when userinfo or the id token has `email_verified === true`. False or missing does not. Consuming a magic link sets it. When an unverified account becomes verified, its password hash is cleared and its other sessions stop working; an already-verified account keeps its password.
...
- Magic link: `/signup` and `/login` tabs `Password` (default) | `Email link`. Email-only + **Email me a link**. Success on the same screen: **Check your email**, hint **Link expires in 15 minutes**, **Resend link** (30s cooldown). Invalid email: **Enter a valid email**. Send failure: **Couldn’t send the link. Try again.** Tokens live in SQLite (`magic_tokens`: hashed token, email, expiresAt, usedAt) — 15 min, one-time, raw token is never stored. Consume is `GET /auth/magic?token=…`. New user → `/onboarding`; returning with a plan → `/today`. Used/expired → `/login` + toast **That link expired. Request a new one.** Mail via Resend (`RESEND_API_KEY` + `MAIL_FROM`; `MAGIC_LINK_FROM` is a one-release fallback). Local/dev with mail unset: the full sign-in URL is logged and the success UI still shows. Production with mail unset: fail with **Couldn’t send the link. Try again.** and never log the token or link. Sign-in URLs use `publicOrigin()` (Railway HTTPS).
- After signup (email, first magic link, or first Google), Continue goes to `/onboarding`. After login (email, returning magic link, or returning Google), Continue goes to `/today` if a plan exists, otherwise `/onboarding`.
- Env: `AUTH_SECRET` (required in production; see `.env.example`). Optional `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_CALLBACK_URL`, `AUTH_DATA_DIR`, `AUTH_COOKIE_SECURE`. Optional magic link: `RESEND_API_KEY`, `MAIL_FROM` (`MAGIC_LINK_FROM` fallback). Adapt job: `ADAPT_CRON_SECRET` (required for `/api/adapt`). Optional LLM (nightly adapt and Generate feedback): `ADAPT_LLM_API_KEY`, `ADAPT_LLM_BASE_URL`, `ADAPT_LLM_MODEL`. `OLLAMA_API_KEY` is a one-release fallback when `ADAPT_LLM_API_KEY` is unset. That fallback ignores `ADAPT_LLM_BASE_URL` and `ADAPT_LLM_MODEL` and calls `https://ollama.com/v1` with `OLLAMA_MODEL` (default `gemma4:31b`). Optional Intervals.icu: `INTERVALS_ICU_API_KEY` (required to Connect; Basic auth user `API_KEY`; HTTP `User-Agent: RunningStatsMVP/0.1`; paths use athlete `0`). `INTERVALS_OWNER_EMAILS` is the comma-separated allowlist (case-insensitive, trimmed) that may use that shared key, and only after `emailVerifiedAt` is set. Unset or empty allows nobody (fail closed). After deploy, Intervals is unavailable for everyone until the owner signs in once with Google or a magic link. Production must set `crispal94@gmail.com`. Optional `INTERVALS_ICU_ATHLETE_ID` is display-only (default Christian `i123456`). The API key is never stored in SQLite or shown in the UI. No separate `SITE` / `APP_URL` — public origin is the request URL / `GOOGLE_CALLBACK_URL`.
...
- Answers, plans, sessions, Baseline, cadence, Feedback, RunLogs, Intervals connection status, and magic-link tokens persist in `.data/app.db` (same `AUTH_DATA_DIR` as users). The Intervals API key is env-only. No AdaptationEvent is written here.
...
- **Settings (`/settings`):** **You** — Adaptation frequency (Daily / Weekly / Monthly, same options as onboarding; Save updates `Plan.feedbackCadence` and the onboarding record). **Connected apps** — Intervals.icu. Every signed-in user sees the same row (`id="intervals"`). Status is **Not connected**, or **Connected as** the athlete name or id (for example **Connected as i123456**). When OAuth is configured, Connect is **Connect Intervals.icu**. The athlete field placeholder is **i123456**. Invalid input shows **Enter an athlete ID like i123456.** under that field. When Connect or Reconnect is disabled, the helper is **Connecting Intervals.icu isn’t available right now. Try again later.** A missing encryption secret or shared key uses that same line. An empty or partial API key form shows **Enter your Intervals API key and athlete ID.** in the row. An OAuth callback that cannot finish because encryption is missing shows that unavailable line rather than **Couldn’t connect to Intervals. Try again.** Connected: **Sync now** and **Disconnect** (Disconnect stops import and does not delete `RunLog`s). Expired access shows **Intervals access expired** and **Reconnect**. Today shows **Intervals access expired. Reconnect to keep your runs and plan in sync.**, with **Reconnect** linking to `/settings#intervals`. Last sync: `Synced 2h ago`, or soft error **Couldn’t sync. Try again.** Import: Run activities only; match Guayaquil day to a Session. Empty fetch / nothing to import for the sync window → toast **No new runs to import**. Runs with no Session that Guayaquil day → skip (no orphan `RunLog`) and toast **No planned session that day** only when that sync or Which run? pick imported nothing. A successful import stays silent even if another activity in the same batch had no Session. If that day has one Session and more than one Run, a **Which run?** sheet lists time + distance (default = closest to the Session target) before upsert. Manual Log this run wins if already saved. Feedback / AdaptationEvent are untouched. **Account** — email and log out.
...
- **If Feedback exists:** write one AdaptationEvent and adjust the **next unfinished Session after today** — a rest day in between is skipped. Never an opaque full Plan rewrite. Idempotent per user + source day.
- **Copy (provisional EN):** `title` is always `Plan adjusted`. `summary` is one line of what changes tomorrow (e.g. Easy run shortened to 5 km). `reason` is one line why (e.g. You skipped Tuesday / Heart rate was high yesterday / Run logged. Tomorrow stays.). No CTL/ATL jargon, no freeform coach chat. The Ollama note is not this reason.
- **Today UI:** chip shows `title` + `summary`. **Why?** opens the sheet with `reason` when non-empty; otherwise the control is omitted. Feeling off says tomorrow's session updates tonight.
- **Heuristic (when `ADAPT_LLM_API_KEY` is unset and the one-release `OLLAMA_API_KEY` fallback is unset):** skip eases to 0.8 and feeling-off to 0.75; intervals/tempo/long become easy. A logged run leaves tomorrow unchanged unless heart rate is at least 95% of LTHR (ease to 0.9 and a hard kind becomes easy), the run was under 85% of planned distance with heart rate at least 90% of LTHR (ease to 0.85), or step rate is under 160 with heart rate at least 90% of LTHR (ease to 0.9). Shorter alone does not change tomorrow. Missing LTHR does not invent a threshold. Cadence from Intervals is one-foot RPM. SQLite store: `.data/app.db` (`AUTH_DATA_DIR`).
- **LLM (optional):** if `ADAPT_LLM_API_KEY` is set (or `OLLAMA_API_KEY` for one release), the job asks an OpenAI-compatible `/chat/completions` endpoint for **typed JSON** (`title`, `summary`, `reason`, `distanceKm`, `kind`). The payload is the planned Session, the heuristic proposal, and the `RunLog` (`distanceKm`, `timeSec`, `paceSecPerKm`, `source`) plus heart rate and cadence when `loadRunEffort` has them. Returned distance is clamped to between half and 20% more than tomorrow. Skip and Feeling off cannot end harder than the heuristic. On failure the heuristic is kept and `llmFailed` increments. The coach note is not sent.
- **Intervals upload:** after a local write, each patched Session is upserted as a Run workout (`POST /athlete/0/events/bulk?upsert=true`, `external_id` = session id, distance in meters) only when that athlete is connected, their email is in `INTERVALS_OWNER_EMAILS`, and `emailVerifiedAt` is set. Unverified allowlisted accounts are skipped the same way as other athletes: they still get the local AdaptationEvent and session patch, and their heart rate is not loaded from Intervals. `uploaded=` counts only those owner uploads. No API key skips the upload. An HTTP or network failure leaves the local plan in place and increments `uploadFailed`. A hold does not upload.
