...
- **Intervals upload:** after a local write, each patched Session is upserted as a Run workout (`POST /athlete/0/events/bulk?upsert=true`, `external_id` = session id, distance in meters) only when that athlete is connected and their email is in `INTERVALS_OWNER_EMAILS`. Other athletes still get the local AdaptationEvent and session patch; their heart rate is not loaded from Intervals. `uploaded=` counts only those owner uploads. No API key skips the upload. An HTTP or network failure leaves the local plan in place and increments `uploadFailed`. A hold does not upload.
